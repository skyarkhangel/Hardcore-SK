...
namespace PeteTimesSix.SimpleSidearms.Utilities
{
    public static class WeaponAssingment
    {
        ...
        public static bool equipSpecificWeapon(Pawn pawn, ThingWithComps weapon, bool dropCurrent, bool intentionalDrop)
        {
            ...
            if (!SimpleSidearms.Settings.AllowBlockedWeaponUse && !StatCalculator.canUseSidearmInstance(weapon, pawn, out string reason))
            {
            ...
            else if (pawn.equipment.Primary != null)
            {
                ...
                if(!addedToInventory)
                    Log.Warning(String.Format("Failed to place primary equipment {0} (initially was {1}) into inventory when swapping to {2} on pawn {3} (colonist: {4}) (dropping: {5}, current drop mode: {6}). Aborting swap. Please report this!",
                       ...
                       pawn?.IsColonist,
                       dropCurrent,
                       SimpleSidearms.Settings.FumbleMode
                    ));
            }
        ...
        public static void DoFumbleMote(Pawn pawn)
        {
            ...
            var chancec = SimpleSidearms.Settings.FumbleRecoveryChance.Evaluate(bestSkillc);
            if (!Prefs.DevMode)
            {
            ...
            else
            {
                ...
                var chance = SimpleSidearms.Settings.FumbleRecoveryChance.Evaluate(bestSkill);
        ...
        public static void equipBestWeaponFromInventoryByPreference(Pawn pawn, DroppingModeEnum dropMode, PrimaryWeaponMode? modeOverride = null, Pawn target = null)
        {
            ...
            string currentJob = pawn.CurJob != null ? pawn.CurJob.workGiverDef != null ? pawn.CurJob.workGiverDef.tagToGive.ToStringSafe() : null : null;
            string lastJob = pawn.mindState.lastJobTag.ToStringSafe();

            //if (currentJob != null && pawn.CurJobDef.defName == "Hunt" || (currentJob == "Idle" || currentJob == "DraftedOrder" || currentJob == "UnspecifiedLordDuty" || currentJob == "Escaping" || currentJob == "SatisfyingNeeds" || currentJob == "TuckedIntoBed" || currentJob == "UnloadingOwnInventory"))
            //{
            // if (pawn.GetTimeAssignment().defName == "Work" || pawn.GetTimeAssignment().defName == "Anything" /* && pawn.MapHeld.weatherManager.RainRate > 0*/)
            //{

                if (lastJob != null && lastJob != "MiscWork" && lastJob != "Fieldwork" && (currentJob == null || currentJob != "MiscWork"))
                //if (lastJob != null && lastJob != "MiscWork" && lastJob != "Fieldwork" && pawn.CurJobDef.casualInterruptible)
                    readyForGun = true;
               // }
            //}
            ...
            if (mode == PrimaryWeaponMode.Ranged && (readyForGun && SimpleSidearms.Settings.RangedNonCombatAutoSwitch || pawn.Drafted) ||
                ((mode == PrimaryWeaponMode.BySkill) && (pawn.getSkillWeaponPreference() == PrimaryWeaponMode.Ranged)) && ((readyForGun && SimpleSidearms.Settings.RangedNonCombatAutoSwitch) || pawn.Drafted) || (huntingNeeded && readyForGun))
            {
                ...
                else
                {
                    ...
                    bool skipDangerous = pawn.IsColonistPlayerControlled && SimpleSidearms.Settings.SkipDangerousWeapons;
                    bool skipEMP = true;
        ...
        internal static void doCQC(Pawn pawn, Pawn attacker)
        {
            ...
            if (SimpleSidearms.Settings.CQCAutoSwitch == true)
            {
                if (attacker != null)
                {
                    ...
                    if (SimpleSidearms.Settings.CQCTargetOnly == true && attacker != pawn.mindState.lastAttackedTarget.Thing)
                    {
                        return;
                    }

                    if (!SimpleSidearms.Settings.OptimalMelee && pawn.equipment.Primary != null && pawn.equipment.Primary.def.IsMeleeWeapon)
                        return;
        ...
        {
            if (!SimpleSidearms.Settings.OptimalMelee || target == null || (target.MentalStateDef == MentalStateDefOf.SocialFighting && pawn.MentalStateDef == MentalStateDefOf.SocialFighting))
                    return;
        ...
        public static bool trySwapToMoreAccurateRangedWeapon(Pawn pawn, LocalTargetInfo target, bool dropCurrent, bool skipManualUse, bool skipDangerous = true, bool skipEMP = true)
        {
            ...
            float currentDPS = StatCalculator.RangedDPS(pawn.equipment.Primary, SimpleSidearms.Settings.SpeedSelectionBiasRanged, bestWeapon.averageSpeed, targetDistance);
