...
namespace PeteTimesSix.SimpleSidearms.Intercepts
{
    ...
    public static class ToilMaker_MakeToil_Patches
    {
        ...
        {
            if (Settings.ToolAutoSwitchBack == true)
            {
                if (toil == null)
                    return;
                ...
                toil.AddPreInitAction(delegate
                {
                    ...
                    bool usingAppropriateTool = false;
                    if (pawn.equipment != null && pawn.equipment.Primary != null && pawn.equipment.Primary.def.modExtensions != null)
                    {
                        usingAppropriateTool = !pawn.equipment.Primary.def.modExtensions.Where(e => e.GetType().FullName == "SurvivalToolsLite.SurvivalToolProperties" || e.GetType().FullName == "SurvivalToolsLite.SurvivalToolHelper").EnumerableNullOrEmpty();
                    }
                    ...
                    {
                        currentJob = pawn.CurJob.workGiverDef.tagToGive.ToStringSafe();
                    }
                    if (currentJob != null && currentJob == "Fieldwork" || currentJob == "MiscWork" || currentJob == "Misc")
                        doingJob = true;

                    if (usingAppropriateTool || doingJob)
                    {
                        if (pawnMemory != null)
                        {
                            //Log.Message("This is Run");
    ...
    public static class PawnRenderer_CarryWeaponOpenly_Postfix
    {
        ...
        {
            //Log.Message("CarryWeaponOpenly PostFix Called");
    ...
    public static class Stance_Warmup_StanceTick_Postfix
    {
        ...
        {
            //Log.Message("StanceTick PostFix Called");
