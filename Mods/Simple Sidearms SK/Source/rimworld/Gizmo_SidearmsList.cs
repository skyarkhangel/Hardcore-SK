...
namespace SimpleSidearms.rimworld
{
    public class Gizmo_SidearmsList : Command
    {
        ...
        public override float GetWidth(float maxWidth)
        {
            ...
            if (!PeteTimesSix.SimpleSidearms.SimpleSidearms.Settings.SettingsEverOpened)
        ...
        public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms parms)
        {
            ...
            if (((Mouse.IsOver(gizmoRect) || Mouse.IsOver(contentRect)) && tickReady) || carriedWeaponsTemp.NullOrEmpty() || update)
            {
            ...
            if (pawnMemory != null)
            {
                ...
                foreach (var group in grouped)
                {
                    ...
                    if(missingCount > 0)
                    {
                        ...
                        //Log.Message("Weapon Added: " + weaponMemory.thing.defName + ". info: " + info.First().Key + ", " + info.First().Value);
        ...
        public void DrawIconForWeapon(Pawn pawn, CompSidearmMemory pawnMemory, ThingWithComps weapon, bool isDuplicate, Rect contentRect, Vector2 iconOffset)
        {
            ...
            bool allowInteraction = StatCalculator.canUseSidearmInstance(weapon, pawn, out string interactionBlockedReason) || PeteTimesSix.SimpleSidearms.SimpleSidearms.Settings.AllowBlockedWeaponUse;
            ...
            if (ev.button == LEFT_CLICK)
            {
                switch (interaction)
                {
                    ...
                    case SidearmsListInteraction.Weapon:
                        ...
                        else if (pawnMemory.DefaultRangedWeapon == weaponType || pawnMemory.PreferredMeleeWeapon == weaponType || (weaponType.isToolNotWeapon() && !PeteTimesSix.SimpleSidearms.SimpleSidearms.Settings.CanSetToolsAsFavourites))
                        {
                        ...
                        else
                        {
                            ...
                            {
                                PlayerKnowledgeDatabase.KnowledgeDemonstrated(SidearmsDefOf.Concept_SimpleSidearmsAdvancedRanged, KnowledgeAmount.SpecificInteraction);
                                pawnMemory.SetRangedWeaponTypeAsDefault(weaponType, false);
                            }
                            else
                            {
                                PlayerKnowledgeDatabase.KnowledgeDemonstrated(SidearmsDefOf.Concept_SimpleSidearmsAdvancedMelee, KnowledgeAmount.SpecificInteraction);
                                pawnMemory.SetMeleeWeaponTypeAsPreferred(weaponType, false);
                            }
                        }
                        break;
                    case SidearmsListInteraction.WeaponMemory:
                        ...
                        else
                        {
                            if (weaponMemory.thing.IsRangedWeapon)
                            {
                                ...
                                pawnMemory.SetRangedWeaponTypeAsDefault(weaponMemory, false);
                            }
                            else
                            {
                                PlayerKnowledgeDatabase.KnowledgeDemonstrated(SidearmsDefOf.Concept_SimpleSidearmsAdvancedMelee, KnowledgeAmount.SpecificInteraction);
                                ...
                                pawnMemory.SetMeleeWeaponTypeAsPreferred(weaponMemory, false);
                            }
                        }
                        break;
