    ...
    public class RewardGeneratorUtilityLGE
    {
        public static List<Thing> GenerateStockpileReward(float singleRewardChance)
        {
            ...
            if (Rand.Chance(singleRewardChance))
            {
                ...
                potentialList.Add(NeurotrainerDefGenerator.ImpliedThingDefs().RandomElement<ThingDef>());

                ThingDef rewardDef;
                potentialList.TryRandomElement(out rewardDef);
                if (rewardDef != null)
                {
                    if (rewardDef != ThingDefOf.AIPersonaCore && rewardDef != ThingDefOf.TechprofSubpersonaCore && Rand.Chance(0.8f))
                    {
