...
namespace RimWorld
{
    public class CompExplosiveNPP : ThingComp
    {
        ...
        {
            SoundStarter.PlayOneShot(CompExplosiveNPP.WickStartSound, (SoundInfo)this.parent.Position);
            SoundInfo info = SoundInfo.InWorld((TargetInfo)((Thing)this.parent), MaintenanceType.PerTick);
            this.wickSoundSustainer = SoundStarter.TrySpawnSustainer(CompExplosiveNPP.WickLoopSound, info);
        }
        ...
        public override void PostDestroy(DestroyMode mode)
        {
            if (mode != DestroyMode.Kill)
                return;
            this.Detonate();
            GenSpawn.Spawn(ThingDef.Named("NPPD"), this.parent.Position);
        }
